...
## Report for FB as of `r Sys.Date()`
...
prices <- round(getSymbols("FB", src = 'google', auto.assign = FALSE), 2)
...
highchart() %>% 
  ...
  hc_add_series(prices, yAxis = 0, name = "FB") %>% 
  hc_add_series(prices[,paste0("FB",".Volume")], name = "Volume", type = "column", yAxis = 1) %>% 
